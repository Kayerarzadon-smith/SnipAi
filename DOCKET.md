...
|---|---|---|
| R12 | **The full journey: "drop raw videos into Snip Cannon > Edits > Asks for review > Approve > post > Snip Cannon generates platform specific captions & hashtags, pulls in the correct affiliate links for tiktok, it attaches your affiliate tiktok shop link every other platform it attaches amazon affiliate link. > and then posts."** | Asked 2026-09-16. **Four separable pieces, not one feature — filing them as one row because he described them as one journey, but they do not ship together.** (1) **Caption + hashtag generation from the finished cut** — new, buildable now, no external dependency; the transcript and beats already exist to draft from. (2) **Platform-correct affiliate link attachment** — TikTok gets his TikTok Shop affiliate link, every other platform gets his Amazon affiliate link. **This is `R2` ("Products & links — the form writes a file only an uninvoked tool reads") becoming load-bearing rather than dormant** — the form that captures those links already exists; nothing reads it yet. Buildable once R2's write path is wired to a read path. (3) **Autonomous posting to TikTok/X/Snapchat/Facebook/Instagram/Pinterest/Trybe/Amazon Storefront** — this is **B1**, still blocked on a developer app and credentials per platform, unchanged tonight. **The pipeline can end at "approved, captioned, linked, ready to post" without B1; it cannot end at "posted" without it.** (4) **The autocut-vs-manual prompt on new-clip upload** — see R13, filed separately since it is a distinct interaction. **Scope call: build 1, 2 and 4 now; 3 stays blocked and the app hands him a ready-to-post file until it clears**  **DEFERRED BY HIM 2026-09-16, same day: "we dont need to do the journey pipeline right now. That might be something for another time."** Kept here rather than deleted -- findable when he wants it, not re-litigated meanwhile | open -- deferred, not in queue |
| R13 | **Prompt for autocut vs. manual cut when new clips are uploaded** | Asked 2026-09-16. A new decision point in the import flow — right now the tray always proposes and auto-drafts. He wants the option to skip the automatic draft and cut a project by hand from the start. Small, contained, sits in `app/dashboard/DropZone.tsx` and the confirm route | open |
| R14 | **"Can you calculate how much time you saved?"** | Asked 2026-09-16, as part of R12's journey. A time-saved figure needs a baseline — his own historical manual-edit time (we have six real versions of `img-9817` with timestamps: 17:05 to 07:07, so a real per-project manual duration exists), or an assumed rate. **His call which baseline to use before this is built**, since the number is meaningless without one he trusts | open |
| R15 | **Rename the app to "Snip Cannon"** | Asked 2026-09-16. **Defaulting to a UI-only rename** — the name shown in the window title, menu bar, and dashboard header — **not** the Xcode project, bundle identifier, `SnipAi.app` path, or the GitHub repo name, all of which every build script, the launcher's ownership check (`native/LaunchDecision.swift`), and this docket's own file paths currently assume. Renaming those too is a real option, just a much bigger and more mechanical change with no user-facing benefit beyond the label — say so if he wants the full rename and it gets its own row | open |
| R16 | **Light and dark theme option** | Asked 2026-09-16. New — nothing in the ledger or docket has touched theming before tonight | open |
| R17 | **"Copy the user interface Scaled Creator (not exactly but model after it.)"** | Asked 2026-09-16. **Not actioned yet — needs either his own reference screenshots or a research pass before any design work starts**, since guessing at another product's UI from memory is exactly the kind of thing that goes wrong quietly. Fastest path is him sending screenshots of what he means; failing that, a look-it-up pass is needed first **DEFERRED BY HIM 2026-09-16: "save the scaled creator interface for another time as well."** | open -- deferred, not in queue |
| R18 | **Animate the loading/progress bar** | Asked 2026-09-16. Cosmetic, contained, in the import tray and build-job progress UI | open |
| R19 | **"It's still snipping but it's saying it's ready for review in the queue window"** | Asked 2026-09-16. **Matches a defect Nadia already found and filed** — a project with no cut built and a job still running shows the "Ready to review" pill regardless (`BUG-001` in her 2026-09-12 report, filed as ledger **S65**). **A screenshot he sent of the same moment shows the underlying mechanism directly**: the import tray's progress panel, verbatim, reading `24.06- 28.64: seen that before I've never seen that before on a monster truck but it makes — about 3 minutes left` as its caption — raw transcript output rather than a phase sentence. That is **S66** (BUG-003, raw transcript lines as the caption), not a new bug, and it is a second, independent sighting of it. Not a new bug; corroborating evidence both fixes are worth doing | open |
| R20 | **"Upon opening this window I'm met with this:"** | Asked 2026-09-16. **Resolved by the screenshot he sent — it is the black-screen player, the same defect as R21.** "This window" is the review screen he lands on after opening a project from the queue, and what he is met with is a fully black player (`0:00.0 / 1:15.1`, a play button, no picture) rather than the first frame. **Merged into R21 — one bug, described across two consecutive sentences of his original message, now confirmed by image rather than guessed at** | see R21 |
| R21 | **First frame of the player should fill the frame, not show a black screen** | Asked 2026-09-16, in his words: "I want the beginning frame to fill the player. Not a black screen." **Confirmed by screenshot, and now covers R20 too**: a fully black 9:16 player, play button centered, `0:00.0 / 1:15.1`, no picture anywhere. The `<video>` element has no `poster` attribute wired to the project's already-generated poster/filmstrip, or the poster route exists and is not being read on first paint (`app/api/projects/[project]/poster`) | open |
| R22 | **"I want the highlight to be on top of the video clip as well"** | Asked 2026-09-16. **Confirmed by two screenshots — two distinct findings, not one.** (1) The beat/caption highlight currently renders on the waveform (audio) row but not on the video-thumbnail row directly above it — one screenshot shows a caption `"This is the classroom"` correctly overlaid on the video thumbnail, while a second label `"jam comes out"` for the neighbouring beat appears only on the waveform beneath, with no matching highlight on the video row. He wants the highlight to appear on **both** tracks together, not just audio. (2) A second, sharper bug in the same screenshots: on a short clip (a **0.96s** selection), the caption tooltip text overlaps and clips itself — `"All right, so here's my"` runs into `"All right... monst[er]"` from an adjacent label, garbled and overflowing its box. That is a **tooltip-overflow-on-short-clips** bug, plausibly the same class as **C30** (a stacking/sizing bug in the player/timeline overlay), but distinct from (1) — the highlight-track issue is a missing feature, the tooltip overlap is a rendering defect | open |
| R23 | **Seamless playback across clip cuts — no pause between one clip and the next** | Asked 2026-09-16, in his words: "There is a slight pause from going from one video clip cut to the next. I do not want it to pause, I want a seamless playback." Actionable without an image. On a joined multi-clip project this is very plausibly a browser re-buffer/seek stall at the seam inside the `<video>` element, distinct from any server-side join defect — M0.8's exit already proved the *file* has no gap or drift at a seam, so if this reproduces, the cause is playback-side rendering, not the joined file itself | open |
| R24 | **"Also it's when I click approve"** | Asked 2026-09-16 — **the message ended there.** Reads like the start of a description of a bug tied to clicking Approve, possibly related to R23's playback pause. Please finish the thought | open — incomplete, needs him to continue |
