...
| B8 | **What is the target length, exactly?** | **M0.85's one open product question, and the second number in two days that must not be guessed into the code.** He said *"about a minute"*, which is a target and not a specification. The three candidate shapes, and they produce different drafters: **(a) 60s as a budget** the drafter optimises toward and may miss; **(b) a band, 45-75s**, which lets a good cut at 68s stop rather than be squeezed; or **(c) "as short as it can be without losing the point"** — no number, and the drafter drops whole beats until meaning breaks, which is the most ambitious and the hardest to verify. **~~Recommendation: (b), a band of 45-75s~~ — WRONG AT BOTH ENDS AND WRONG IN SHAPE, corrected 2026-09-11 against eight of his own videos, and the band was my recommendation so the error is mine.** Measured: **37.40, 40.97, 44.14, 45.90, 54.45, 57.00, 57.45, 83.40** — **two below 45, one above 75, so 45-75 would have excluded three of his eight**, and the two it excluded at the bottom are the tightest work in the set. **But the endpoints are the smaller error. The shape is wrong:** he runs 37s to 83s depending on what the video is, so a band encodes a constraint he does not hold, and a hard ceiling would cut into `bbe0c896` — his own 83-second video. **I argued against option (c) on the grounds it was hardest to verify; the evidence says (c) was right about the shape.** **New recommendation: a target near his median (~50s) with no hard ceiling, and a stop rule rather than a limit** — the drafter shortens while it is dropping *duplicates* and stops when it would start dropping *content*. **And do not compile the number at all:** `app/api/references/route.ts:167` already derives a house style as *"measure_finished.py across reference/inspiration — median of them all"*, so **the target should be measured from his own reference videos rather than chosen by anyone** — which also makes the 15% outlier harmless, since a median absorbs it. **Still his to answer**, and he now has eight data points instead of my guess. The one-sentence argument: a hard 60 would force the drafter to cut into a good 68-second video, and a band is the only one of the three that can say *"this is finished"* rather than *"this is as close as I got"* — which is the difference between the six versions of `img-9817` and one. **B7's lesson applies directly and is four hours old: a number we choose becomes a number he has to fight.** Waiting on him |
