...
| M3 | Finish the three-level review loop | Level 1 → 2 → 3 usable without knowing `m` is a shortcut . **Scope audited 2026-09-11 and it is larger than three rows.** None of H1-H9 is silently finished. H1 needs an exit condition it lacked — *diagnose a beat, reload, see your own answer* (`beatDiagnoses` has no reader today, U9's write-only half). H2 is an undone decision: restore `needs_fixes` **with a destination** or delete the branch. H3's real defect is that `setLevel2Open(false)` has no call site, so the panel cannot be dismissed once opened. **C5 and C12 land inside this work and should be fixed in the same pass.** **Decide here, not later:** P6's existing Level-3 caller is `saveGraphicNote` posting `checks: ["needs-footage"]`, so the panel will render graphics requests as diagnoses of the line — settle whether that belongs in `beatDiagnoses` at all rather than discovering it when it appears against lines nobody diagnosed. **Gated on the clamp/layout answer from Kayer**: at his 900pt display the lines list is below the fold for the whole cut during playback, and building the review loop on a screen that cannot show both halves is building on the wrong foundation | H1, H2, H3, H10; C5, C12 in the same pass |
