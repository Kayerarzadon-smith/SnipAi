...
**Ownership note, 2026-09-11: M0.8 is owned by another session.** It is live
on master, its design in the row below is well-argued, and it is **not
available work**. An open milestone with no visible owner reads as free to
pick up, and two sessions inside one feature is what produced this project's
worst process failure to date — an in-flight working tree read and filed as
`fixed`, and three commits landing on a branch mid-merge. If you are reading
this file for what to take next, skip M0.8 and take the first row below it
that is not claimed.
