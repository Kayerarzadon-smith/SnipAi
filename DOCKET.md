...
| R19 | **"It's still snipping but it's saying it's ready for review in the queue window"** | Asked 2026-09-16. **Matches a defect Nadia already found and filed** — a project with no cut built and a job still running shows the "Ready to review" pill regardless (`BUG-001` in her 2026-09-12 report, filed as ledger **S65**). Not a new bug; this is him hitting it independently, which is corroborating evidence the fix is worth doing | open |
| R20 | **"Upon opening this window I'm met with this:" [image not received]** | Asked 2026-09-16. **The image did not come through — it shows as an empty attachment on this end.** Please resend; cannot act on this until I can see it | open — blocked on the image |
| R21 | **First frame of the player should fill the frame, not show a black screen** | Asked 2026-09-16, in his words: "I want the beginning frame to fill the player. Not a black screen." Actionable without an image — the player is showing black before playback starts instead of a poster frame. Likely the `<video>` element has no `poster` attribute wired to the project's already-generated poster/filmstrip, or the poster route exists and is not being read on first paint (`app/api/projects/[project]/poster`) | open |
| R22 | **"I want the highlight to be on top of the video clip as well"** [image not received] | Asked 2026-09-16. **The image did not come through**, so the exact highlight he means is not confirmed. Likely candidate, stated as a guess and not a diagnosis: a stacking/z-index bug where a selection or overlay element renders behind the video layer — the same class as **C30**, where the Play button was clipped out of its own box by `aspect-ratio` + `overflow: hidden` on the player container. Please resend the image before this is scoped further | open — blocked on the image |
