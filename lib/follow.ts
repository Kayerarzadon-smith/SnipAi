...
export type FollowInput = {
  ...
  playing: boolean;
  /** `.stage` bottom edge in viewport coordinates, null when there is no stage */
...
export function followScroll(i: FollowInput): ScrollRequest {
  ...
  const want = i.stageBottom === null ? 140 : i.stageBottom + 12;
  const delta = i.rowTop - want;
...
export function revealScroll(i: RevealInput): ScrollRequest {
  const hidden = i.rowTop < 60 || i.rowBottom > i.viewportHeight - 40;
